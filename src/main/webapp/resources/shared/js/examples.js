    ...
    Application = {
        ...
        fetch: function (href, forced) {
            ...
            $.get(href, {partial: 1}, function (html) {
                exampleWrap.kendoStop(true).kendoAnimate(extend({}, animation.hide, {
                    complete: function () {
                        var items = $("#examples-nav li").removeClass("active"),
                            item = $($.grep(items, function (li) {
                                return href.indexOf($(li).find("a").attr("href")) > -1;
                            })).addClass("active");
        ...
        fetchWidget: function (href) {
            ...
            $.get(href + "?nav=true", function (html) {
                ...
                wrapInner.kendoStop(true).kendoAnimate(extend({}, animation.hide, {
                    complete: function () {
        ...
        ThemeChooser = Widget.extend({
            init: function (element, options) {
                ...
                this.element.on("click touchend", ".tc-link", $.proxy(function (e) {
                    ...
                    this._getThemeContainer().animate({
                        height: "toggle",
                        margin: "toggle",
